...
- **write_file reconcile**（ADR-0011）：读目标文件比对内容——内容一致 → `succeeded {path, verified: true, bytes}`（写入确已发生）；文件不存在或内容不同 → `failed`"the write did not take effect"（确证未发生）。只验证、零副作用：崩溃遗留的 `.praxis-tmp-*` 不清理（清理由 M3-T004 恢复编排决定），验证不改变 mtime。
...
- 安全：`tests/security/tools-local.security.test.ts`（读：相对/绝对路径逃逸、symlink 逃逸、越界 list_dir；写：traversal/绝对路径/symlink 文件/symlink 目录逃逸全部拒绝且 root 外零写入、reconcile 逃逸路径不授予任何东西、bash cwd/诚实边界）。门：`mise run test:security`。
