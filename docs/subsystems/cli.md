...
- **命令**（零依赖手写 argv 解析，`--flag value` 对 + 布尔旗标 `--allow-write`/`--allow-bash`）：
  - `run [--db P] [--session ID] [--input TEXT] [--root DIR] [--allow-write] [--allow-bash] ( --script FILE | --model NAME [--api-key KEY] [--base-url URL] )`：三态合一——无 `--session` 创建会话（需 `--input`）；`--session + --input` 开新 turn；仅 `--session` 续跑 open turn（含崩溃恢复）。两个模型源互斥；`--model` 走 `@praxis/provider-openai`（key 回退 `OPENAI_API_KEY`，缺 key 拒绝启动；key 只进请求头，不进事件/输出）。退出码 0 完成 / 1 用法或运行错误 / 2 paused 或 cancelled（turn 保持 open，提示 resume 命令）。
  - `sessions [--db P]`：列出会话 id、状态、head seq（SQLite 元数据投影，事实以事件流为准）。
  - `help` / 未知命令：usage 到 stderr。
- **工具与授权（M8-T002）**：read 工具与 write 工具（write_file/bash，均声明 workspace-scoped `requiredCapability`）恒常注册；每次 `runTurn` 传入 `capabilityAuthorizer`（core 的 policy 语义，CLI 只组装）：`--allow-write` 授 `fs.write`、`--allow-bash` 授 `shell.exec`（grant scope = `--root`，相互独立，仅本次调用有效）；未授权的写尝试落为 durable `ToolRejected`（requires_approval，fail closed）——不产生文件系统效果，也绝不静默。approval UX（交互式批准/lease）按 ADR-0007 属扩展，CLI 不内置。
...
- 快速上手（read/write/approval/resume 全流程，复制即可运行）：`docs/13-cli-quickstart.md`。
