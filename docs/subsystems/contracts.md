...
- **v1 事件词汇**（`src/events/session-events.ts` + `src/events/tool-events.ts`）：Session/Turn 生命周期切片（`SessionCreated(reason?)`, `SessionResumed`, `SessionPaused`, `SessionCompleted`, `TurnStarted(turnId)`, `TurnCompleted(turnId)`）+ 工具执行生命周期切片（M2-T003 起：`ToolProposed(toolExecutionId, name, argumentsJson, effect)`, `ToolAuthorized`, `ToolRejected(reason)`, `ToolStarted`, `ToolSucceeded(resultJson)`, `ToolFailed(message)`, `ToolIndeterminate(reason)`）。判别联合 `SessionEventUnionSchema` 拒绝词汇外类型。模型调用/证据事件在 M2–M4 以带 schemaVersion 的成员加入。
- **工具词汇与 port**（`src/tools/tool-effect.ts` + `src/ports/tool.ts`，M2-T003 起）：`ToolEffect = read_only | idempotent_write | reconcilable_write | non_idempotent_write`，作为提案事实记录。`ToolDefinition` port：name/description/effect/inputSchema（从 `unknown` 解析）/`execute(ctx, input)` 返回 `succeeded(resultJson) | failed(error) | indeterminate(reason)`——结果是不透明 JSON 事实，运行时不解析；`failed` 仅当效果确证未发生，未知结果必须 `indeterminate`（docs/02 §8.2 硬规则）。
...
- 单元：`tests/tool-events.test.ts`（工具事件 schema 校验、effect 枚举、必填 reason/message、词汇外拒绝）。
...
- 属性：`tests/property/tool-events.property.test.ts`（工具事件 JSON 往返恒等、result payload 往返保真）。门：`mise run test:property`。
