...
class WorkflowPolicyTest(unittest.TestCase):
    ...
    def test_finish_output_excludes_compile_verification_step_and_checks_feature_upstream(self) -> None:
        ...
        self.assertIn("git branch --unset-upstream", text)
        self.assertIn("git branch -vv", text)
        self.assertIn("cd <project-main-repo>", text)
        self.assertIn("git switch -c feature/氧化上下梁识别问题 origin/develop", text)
        self.assertIn("git cherry-pick abc123", text)
        self.assertIn("Push is intentionally omitted", text)
        self.assertIn("user-only: git push -u origin feature/氧化上下梁识别问题", text)
        self.assertNotIn("\n  git push", text)
        self.assertNotIn("git cherry-pick abc123 bad999", text)
