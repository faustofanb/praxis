...
def context_command(config: dict[str, Any], project: str, requirement_name: str) -> None:
    ...
    print("  所有工作流命令必须优先使用 task project/task gate/task system 等已封装入口；没有对应入口就是工作流缺口，先回报缺口，不得降级裸跑。")
    print("  Codeup Git 状态、差异、分支、提交和推送必须走工作流入口；未经用户明确允许，禁止手动执行 git、rtk git 或 /usr/bin/git。")
    ...
    print("  跨模块或不熟悉代码时，worker 优先使用 CodeGraph；不可用时再查 Praxis Code Graph：")
    print("    - 当前 worktree 存在 .codegraph/ 且 codegraph CLI 可用时，先运行 task system -- codegraph explore <问题> 或 task system -- codegraph impact <符号>。")
