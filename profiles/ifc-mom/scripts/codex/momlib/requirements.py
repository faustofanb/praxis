...
def assert_requirement_binding(requirement_name: str, req_dir: Path, worktree_path: Path | None = None) -> None:
    ...
    if worktree_path.name != f"{req_dir.name}-dev" or not current_branch.startswith("codex/") or not current_branch.endswith(expected_suffix):
        fail(
...
def start_requirement(config: dict[str, Any], project: str, requirement_name: str, raw_requirement: str = "") -> None:
    ...
    assert_requirement_binding(requirement_name, req_dir)
    worktree_path = None
    if project != "docs":
        worktree_path = create_worktree(config, project, requirement_name, None)
        assert_requirement_binding(requirement_name, req_dir, worktree_path)
