...
def finish_requirement(config: dict[str, Any], project: str, requirement_name: str) -> None:
    ...
    print(f"  git fetch origin {upstream}")
    print(f"  git switch -c {feature_branch} {feature_base}")
    print("  git branch --unset-upstream")
    print("  git branch -vv")
    ...
    if included_commits:
        ...
        print("  git cherry-pick <需求提交哈希>")
    print("  # Push is intentionally omitted. Codex must not push; the user pushes after review.")
    print(f"  # user-only: git push -u origin {feature_branch}")
    print()
    print("Requirement worktree cleanup after feature delivery is confirmed:")
    print("  git -C <requirement-worktree-path> status --short")
    print(f"  {praxis_contracts.praxis_usage(f'delivery cleanup {project} {requirement_name}')}")
    print("  # equivalent low-level cleanup:")
    print("  git worktree remove <requirement-worktree-path>")
    print("  git worktree remove <legacy-feature-worktree-path>  # only if an older deliver created one")
