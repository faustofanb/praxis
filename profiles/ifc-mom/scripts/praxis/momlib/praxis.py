...
def _is_command_legacy(raw: str) -> bool:
    ...
    tokens = raw.split()
    try:
        index = tokens.index("scripts/praxis/task.py")
    except ValueError:
...
def _scan_command_examples() -> tuple[list[str], list[str]]:
    ...
    def add(raw: str) -> None:
        ...
        if not raw:
            return
        if "scripts/praxis/task.py" not in raw and " task " not in f" {raw} ":
            return
        command = raw
    ...
    for path in (ROOT_DIR / "AGENTS.md", ROOT_DIR / "README.md"):
        ...
        for line in path.read_text(encoding="utf-8").splitlines():
            if "scripts/praxis/task.py" not in line:
                continue
            for fragment in line.split():
                if "scripts/praxis/task.py" not in fragment:
                    continue
                add(fragment)
...
def command_audit(mode: str = "auto") -> Path:
    ...
    report = {
        ...
        "commandPattern": "task <group> ... / uv run scripts/praxis/task.py <group> ...",
