...
from .git_worktree import action_repo_dir, is_development_branch, project_worktree_dirs
...
def finish_requirement(config: dict[str, Any], project: str, requirement_name: str) -> None:
    ...
    print("  # Keep reviewed requirement commits in the local Praxis worktree; create feature in the main project repo.")
    ...
    print("  # Push is intentionally omitted. The workflow must not push; the user pushes after review.")
    ...
    if is_development_branch(config, current_branch):
...
def cleanup_requirement(config: dict[str, Any], project: str, requirement_name: str) -> None:
    """删除需求 worktree、需求开发分支，并 prune Git worktree 元数据。"""
    repo_dir = project_dir(config, project)
    worktree_dirs = [
        path
        for path in project_worktree_dirs(config, project, requirement_name, include_feature=True)
    ...
    for worktree_dir in worktree_dirs:
        ...
        if is_development_branch(config, current_branch) and current_branch not in development_branches:
            development_branches.append(current_branch)
