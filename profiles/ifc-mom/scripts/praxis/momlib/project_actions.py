...
RUNTIME_DIR = ROOT_DIR / "scripts" / "praxis"
...
def verify_project(config: dict[str, Any], name: str, args: list[str]) -> None:
    ...
    repo_dir = action_repo_dir(config, name, args)
    run_exit(["uv", "run", str(RUNTIME_DIR / "verify.py"), name, "--repo", str(repo_dir)], ROOT_DIR)
...
def run_project(config: dict[str, Any], name: str, args: list[str]) -> None:
    ...
    repo_dir = action_repo_dir(config, name, args)
    if name == "backend":
        run_exit(["uv", "run", str(RUNTIME_DIR / "backend_run.py"), name, "--repo", str(repo_dir)], ROOT_DIR)
