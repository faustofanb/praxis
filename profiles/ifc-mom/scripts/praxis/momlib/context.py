...
def context_command(config: dict[str, Any], project: str, requirement_name: str) -> None:
    ...
    print("  微小参数变更快速通道：常量、参数 key、默认值、SQL 模板或已有页面参数切换只做变更范围检查。")
    print("  恢复已有工作树后修改，执行 git diff --check、变更文件 ESLint/Java compile、适用时 SQL EXPLAIN，最多 1–2 条契约测试。")
    print("  生产代码最后一次修改后只执行一次项目 verify；之后只新增测试时仅重跑对应测试类。")
    print("  后端禁止手动执行根级 Maven -am install；只有发布本地 Maven 制品或跨仓库联调时允许 install。")
    print("  禁止使用浏览器、桌面控制或电脑控制工具执行测试；不自动访问管理页面、读取登录态、操作外部系统或发布结果。")
    print("  浏览器/UI/人工运行时测试、外部联调、完整回归或额外测试或复核只能列为可选项，得到用户明确许可后才能执行。")
    print()
    print("按需规则:")
