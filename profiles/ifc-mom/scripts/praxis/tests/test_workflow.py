...
from momlib import finish, git_worktree, praxis, process, project_actions, quick_tasks, requirements  # noqa: E402
...
class WorkflowPolicyTest(unittest.TestCase):
    ...
    def test_verify_uses_praxis_runtime_script(self) -> None:
        config = {"projects": {"backend": {"kind": "code", "path": "backend"}}}

        with (
            patch.object(project_actions, "action_repo_dir", return_value=Path("/repo")),
            patch.object(project_actions, "run_exit") as run_exit,
        ):
            project_actions.verify_project(config, "backend", ["修复"])

        command = run_exit.call_args.args[0]
        self.assertEqual(Path(command[2]), project_actions.ROOT_DIR / "scripts/praxis/verify.py")

    def test_backend_run_uses_praxis_runtime_script(self) -> None:
        config = {"projects": {"backend": {"kind": "code", "path": "backend", "run": "echo ok"}}}

        with (
            patch.object(project_actions, "action_repo_dir", return_value=Path("/repo")),
            patch.object(project_actions, "run_exit") as run_exit,
        ):
            project_actions.run_project(config, "backend", ["修复"])

        command = run_exit.call_args_list[0].args[0]
        self.assertEqual(Path(command[2]), project_actions.ROOT_DIR / "scripts/praxis/backend_run.py")

    def test_project_worktree_dir_rejects_ambiguous_matches(self) -> None:
        with patch.object(
            git_worktree,
            "project_worktree_dirs",
            return_value=[Path("/worktrees/one"), Path("/worktrees/two")],
        ):
            with self.assertRaises(SystemExit) as raised:
                git_worktree.project_worktree_dir({}, "backend", "修复")

        self.assertEqual(raised.exception.code, 1)

    def test_quick_worktree_path_does_not_require_requirement_docs(self) -> None:
        config = {
            "worktreeRoot": ".worktrees",
            "projects": {"web": {"path": "apps/web", "kind": "pnpm-web"}},
        }

        with patch.object(git_worktree, "find_requirement_dir") as find_requirement_dir:
            path = git_worktree.new_worktree_path(
                config,
                "web",
                "修复标签",
        ...
        find_requirement_dir.assert_not_called()
        self.assertEqual(
            path,
            git_worktree.ROOT_DIR / ".worktrees/apps-web/2026-07-19-修复标签-dev",
        )

    def test_worktree_creation_lock_rejects_active_owner(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            path = Path(tmp_dir) / "web" / "task-dev"
            lock_path = path.parent / ".locks" / "web-task.lock"
            lock_path.parent.mkdir(parents=True)
            lock_path.write_text(f"{os.getpid()}\n", encoding="utf-8")

            with self.assertRaises(SystemExit) as raised:
                with git_worktree.worktree_creation_lock(path, "web", "task"):
                    self.fail("lock should not be acquired")

        self.assertEqual(raised.exception.code, 1)

    def test_worktree_creation_lock_does_not_steal_fresh_incomplete_lock(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            path = Path(tmp_dir) / "web" / "task-dev"
            lock_path = path.parent / ".locks" / "web-task.lock"
            lock_path.parent.mkdir(parents=True)
            ...
            with self.assertRaises(SystemExit):
                with git_worktree.worktree_creation_lock(path, "web", "task"):
                    self.fail("fresh lock should not be stolen")

    def test_start_quick_task_writes_resumable_state_without_docs(self) -> None:
        config = {"projects": {"web": {"path": "apps/web", "kind": "pnpm-web"}}}
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            manifest = root / ".praxis/extensions/ifc-mom/manifest.toml"
...
""".strip(),
            ...
            worktree = root / ".worktrees/apps-web/2026-07-19-修复标签-dev"
            with (
                patch.object(quick_tasks, "ROOT_DIR", root),
                patch.object(quick_tasks, "create_worktree", return_value=worktree) as create_worktree,
                patch.object(quick_tasks, "branch_today", return_value="20260719"),
            ):
                quick_tasks.start_quick_task(config, "web", "修复标签")

            state = root / ".praxis/tasks/20260719-修复标签.toml"
            self.assertTrue(state.is_file())
            self.assertIn('verification_level = "L0"', state.read_text(encoding="utf-8"))
            self.assertIn('checks = ["changed-file-boundary", "syntax"]', state.read_text(encoding="utf-8"))
    ...
    def test_check_quick_task_rejects_high_risk_changed_files(self) -> None:
        config = {"projects": {"web": {"path": "apps/web", "kind": "pnpm-web"}}}
        with (
            patch.object(quick_tasks, "project_worktree_dir", return_value=Path("/worktree")),
            patch.object(quick_tasks, "changed_files", return_value=["src/reports/AuditReport.ts"]),
            self.assertRaises(SystemExit) as raised,
        ):
            quick_tasks.check_quick_task(config, "web", "修复标签")
...
"""
    ...
    def test_cleanup_removes_requirement_worktree_and_praxis_branch(self) -> None:
        config = {
            "projects": {
                "backend": {
                    "path": "ifc-mom-column-max",
        ...
        def fake_capture(command: list[str], cwd: Path) -> str:
            text = " ".join(command)
            if "branch --show-current" in text:
                return "praxis/20260512-氧化上下梁识别问题"
            if "status --short" in text:
                return ""
            return ""
        ...
        self.assertEqual(
            ...
            [
                ...
                ["git", "-C", "/repo", "branch", "-D", "praxis/20260512-氧化上下梁识别问题"],
                ["git", "-C", "/repo", "worktree", "prune"],
            ],
        )
    ...
    def test_project_quick_routes_without_requirement_document(self) -> None:
        config = {"projects": {"web": {"path": "apps/web", "kind": "pnpm-web"}}}
        with (
            patch.object(task_module, "load_config", return_value=config),
            patch.object(task_module, "start_quick_task", create=True) as start_quick_task,
        ):
            task_module.main(["project", "quick", "web", "修复标签"])

        start_quick_task.assert_called_once_with(config, "web", "修复标签")

    def test_project_quick_check_routes_to_changed_file_boundary(self) -> None:
        config = {"projects": {"web": {"path": "apps/web", "kind": "pnpm-web"}}}
        with (
            patch.object(task_module, "load_config", return_value=config),
            patch.object(task_module, "check_quick_task", return_value=0) as check_quick_task,
        ):
            task_module.main(["project", "quick-check", "web", "修复标签"])

        check_quick_task.assert_called_once_with(config, "web", "修复标签")
    ...
    def test_workflow_git_subprocesses_use_rtk_when_available(self) -> None:
        completed = __import__("subprocess").CompletedProcess(["rtk", "git", "status"], 0, stdout="")

        with patch.object(process.shutil, "which", return_value="/opt/homebrew/bin/rtk"):
            with patch.object(process.subprocess, "run", return_value=completed) as run:
                process.run_checked(["git", "status"], Path("/tmp"))

        command = run.call_args.args[0]
        self.assertEqual(command, ["/opt/homebrew/bin/rtk", "git", "status"])

    def test_machine_readable_git_capture_bypasses_rtk_filtering(self) -> None:
        completed = __import__("subprocess").CompletedProcess(["/usr/bin/git", "status"], 0, stdout="")

        with patch.object(process.shutil, "which", return_value="/opt/homebrew/bin/rtk"):
            with patch.object(process.subprocess, "run", return_value=completed) as run:
                process.capture(["git", "status", "--short"], Path("/tmp"))

        command = run.call_args.args[0]
        self.assertEqual(command, ["/usr/bin/git", "status", "--short"])

    def test_workflow_git_subprocesses_fall_back_to_system_git_without_rtk(self) -> None:
        completed = __import__("subprocess").CompletedProcess(["/usr/bin/git", "status"], 0, stdout="")
