...
description: '用于维护 IFC MOM adapter 的 Praxis 命令契约一致性。适用于修改 Taskfile.yml、.praxis/commands.toml、.praxis/manifest.toml、scripts/codex/task.py、AGENTS.md、Praxis 规则文档或任何 task/命令入口说明，防止文档命令与脚本命令漂移。'
user-invocable: true
---
...
- `task ...` 是对话和文档中的首选入口；底层 `uv run scripts/codex/task.py ...` 只作为实现细节或测试入口。
...
1. 先定位命令真实实现：`Taskfile.yml`、`scripts/codex/task.py` 和对应 `momlib/*` parser。
...
uv run scripts/codex/tests/test_praxis.py
uv run scripts/codex/tests/test_workflow.py
uv run scripts/codex/tests/test_praxis_profile.py
uv run scripts/codex/tests/test_praxis_templates.py
```
